...
void score2file(){
	...
	if (f != NULL){
		...
		fprintf(stderr,"Impossible d'ouvrir le fichier des scores");
}
...
int* sort_scores(FILE* file){
	static int* file_scores = NULL;
	file_scores = malloc(sizeof(int)*10);

	if (file != NULL){
		file_scores = get_scores(file);
		int triee = 0;
        int taille = 10;
        int i=0;
        while (!triee){
                ...
                for (i=0; i < taille-1; i++){
                        if (file_scores[i]<file_scores[i+1]){
                                file_scores[i] = file_scores[i] ^ file_scores[i+1];
                                file_scores[i+1] = file_scores[i] ^ file_scores[i+1];
                                file_scores[i] = file_scores[i] ^ file_scores[i+1];
                                triee=0;
                        }
                }
